...
async function executePython(code, input, jobId) {
  ...
  const dockerCommand = `docker run --rm -v "${TEMP_DIR}:/app" -w /app --memory=${process.env.MAX_MEMORY || '128m'} --cpus="${process.env.MAX_CPU || '0.5'}" --network none python:3.9-slim timeout 10s python ${jobId}.py < ${jobId}_input.txt`;
...
async function executeCpp(code, input, jobId) {
  ...
  const dockerCommand = `docker run --rm -v "${TEMP_DIR}:/app" -w /app --memory=${process.env.MAX_MEMORY || '128m'} --cpus="${process.env.MAX_CPU || '0.5'}" --network none gcc:9 bash -c "g++ ${jobId}.cpp -o ${jobId} && timeout 5s ./${jobId} < ${jobId}_input.txt"`;
...
async function executeJavaScript(code, input, jobId) {
  ...
  const dockerCommand = `docker run --rm -v "${TEMP_DIR}:/app" -w /app --memory=${process.env.MAX_MEMORY || '128m'} --cpus="${process.env.MAX_CPU || '0.5'}" --network none node:16-slim node ${jobId}.js < ${jobId}_input.txt`;
...
async function executeJava(code, input, jobId) {
  ...
  const dockerCommand = `docker run --rm -v "${TEMP_DIR}:/app" -w /app --memory=${process.env.MAX_MEMORY || '128m'} --cpus="${process.env.MAX_CPU || '0.5'}" --network none openjdk:11-slim bash -c "javac ${jobId}.java && timeout 5s java ${path.parse(jobId).name} < ${jobId}_input.txt"`;
...
function executeDockerCommand(command) {
  return new Promise((resolve) => {
    const timeout = parseInt(process.env.DOCKER_TIMEOUT) || 15000;
    ...
    exec(command, { timeout }, (error, stdout, stderr) => {
      if (error) {
        ...
        } else if (stderr && (stderr.includes('compilation terminated') || stderr.includes('error:'))) {
          resolve({
            success: false,
